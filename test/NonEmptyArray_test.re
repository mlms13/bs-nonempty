...
describe("NonEmptyArray Basics", () => {
  ...
  test("Head of singleton", () =>
    expect(head(single)) |> toEqual(0)
  );
  test("Tail of singleton", () =>
    expect(tail(single)) |> toEqual([||])
  );
  test("Tail of two-value NE Array", () =>
    expect(tail(tuple)) |> toEqual([|1|])
  );
  test("Length of singleton", () =>
    expect(length(single)) |> toEqual(1)
  );
  test("Length of two-value NE Array", () =>
    expect(length(tuple)) |> toEqual(2)
  );
});
